...
#ifdef _hardwareserial_h 
	#undefine _hardwareserial_h
#endif
...
class SckBase
{
	private:
		...
		// Update Display
		uint32_t updateDisplayMillis=0;

		// GPS Location update
		uint32_t updateGPSMillis=0;
		
		// SCD4x CO2   update
		uint32_t updateSCD4xMillis=0;
		...
		const uint16_t DYNAMIC_HDOP_THRESHOLD = 350; 	// If HDOP is greater than this we ignore the reported speed (this help cleaning false high speed dynamic trigger's)
	...
	public:
		...
		const String SAMversion	= SAMverNum + "-" + String(__GIT_HASH__); 		// major.minor.build-gitcommit
		const String SAMbuildDate = String(__ISO_DATE__);			// this is defined by platform.ini (which produces a JSON file containing __ISO_DATE__ having 3 quotes )
		...
		bool disableSensor(SckBase *base, SensorType wichSensor);
