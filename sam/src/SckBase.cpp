...
void SckBase::setup()
{
	...
	// SD card
	sckOut("Setting up SDcard interrupt",PRIO_LOW,true);
	attachInterrupt(pinCARD_DETECT, ISR_sdDetect, CHANGE);
	sdDetect();

	// Flash storage
	sckOut("Starting flash memory...",PRIO_LOW,true);
	...
	if (rcode == 1) sckOut("Found problems on flash memory, it was formated...",PRIO_MED,true);
	else if (rcode == -1) {
		while (true) {
			sckOut("Error starting flash memory!!!",PRIO_MED,true);
			delay(1000);
		}
	}
...
/* #define autoTest  // Uncomment for doing Gases autotest, you also need to uncomment  TODO complete this */
	...
	// try to start enabled auxiliary bus boards
...
bool SckBase::tcaMuxMode() {
	return auxBoards.TCAMUXMODE;
}
uint8_t SckBase::listMuxChanMap() {
	return auxBoards.listMuxChanMap(this);
}
uint8_t SckBase::listMuxChanMap(SckBase* base) {
	return auxBoards.listMuxChanMap(base);
}
void SckBase::update()
{
...
// **** Sensors
void SckBase::urbanStart(SckBase *base)
{
	...
	if (urban.present()) {
		...
		sckOut("No urban board detected!!",PRIO_MED,true);

		// Disable all urban sensors
		for (uint8_t i=0; i<SENSOR_COUNT; i++) {
			OneSensor *wichSensor = &sensors[static_cast<SensorType>(i)];
			if (wichSensor->location == BOARD_URBAN && wichSensor->enabled) disableSensor(base,wichSensor->type);
		}
	}
}
void SckBase::updateSensors()
{
	...
	if (!rtc.isConfigured() || now < 1514764800) {		// what time is this ?
...
{
	sckOut("SckBase::enableSensor: start point",PRIO_LOW,true);
	sprintf(outBuff,"Trying to enable: %s at location: %3i",sensors[wichSensor].title,sensors[wichSensor].location);
	sckOut(PRIO_LOW,true);
	bool result = false;
	switch (sensors[wichSensor].location) {
		case BOARD_BASE:
		{
			sckOut("SckBase::enableSensor: BOARD_BASE called",PRIO_LOW,true);
			switch (wichSensor) {
...
bool SckBase::disableSensor(SckBase *base, SensorType wichSensor)
{
	...
	switch (sensors[wichSensor].location) {
		...
		case BOARD_AUX: if (auxBoards.stop(base,wichSensor)) result = true; break;
		default: break;
	}
...
bool SckBase::controlSensor(SensorType wichSensorType, String wichCommand)
{
	...
	if (sensors[wichSensorType].controllable)  {
		...
		switch (sensors[wichSensorType].location) {
				...
				case BOARD_AUX: sckOut(auxBoards.control(this, wichSensorType, wichCommand)); break;
				default: break;
			}
...
bool SckBase::sdPublish()
{
	...
	sckOut("ERROR failed publishing to SD card",PRIO_MED,true);
