...
void sensorConfig_com(SckBase* base, String parameters)
{
	...
	if (parameters.length() <= 0) {
		...
		if (parameters.indexOf("-enable") >=0) {
			...
			base->disableSensor(base, sensorToChange);
...
/*
The following function added by Bryn Parrott (bryn.parrott@gmail.com)
to support the potential inclusion of the I2C 8 channel Mux unit on the Auxilliary I2C bus.
It started as a clone of i2cDetect_mux
This version Adds support to display the I2C map that (may have) been discovered
during startup of the AuxBoards class.
*/
	
void i2cDetect_mux_com(SckBase* base, String parameters)
{
