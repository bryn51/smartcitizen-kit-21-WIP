...
enum SensorType
{
	...
	// Actuators (This is temporary)
	SENSOR_GROVE_OLED,

	// SCD4x : An Alternative to SCD30 that uses less physical space
	SENSOR_SCD4x_CO2,
	SENSOR_SCD4x_TEMP,
	SENSOR_SCD4x_HUM,
	...
	SENSOR_RAIN_ACC,
	SENSOR_RAIN_EVENTACC,
	SENSOR_RAIN_TOTALACC,
	SENSOR_RAIN_INTERVAL,
...
class AllSensors
{
	public:
		...
		OneSensor list[SENSOR_COUNT+1] {
			...
			OneSensor { BOARD_AUX,		100,	SENSOR_INA219_BUSVOLT,			"INA_VBUS",		"INA219 Bus voltage",				165,		true,		false,		1,			"V"},
			OneSensor { BOARD_AUX,		100,	SENSOR_INA219_SHUNT,			"INA_VSHUNT",		"INA219 Shunt voltage",				166,		true,		false,		1,			"mV"},
			OneSensor { BOARD_AUX,		100,	SENSOR_INA219_CURRENT,			"INA_CURR",		"INA219 Current",				167,		true,		false,		1,			"mA"},
			OneSensor { BOARD_AUX,		100,	SENSOR_INA219_LOADVOLT,			"INA_VLOAD",		"INA219 Load voltage",				168,		true,		false,		1,			"V"},
			...
			// Groove I2C Oled Display 96x96  (Priority value of 250 means it will not be updated in the normal spin)
			OneSensor { BOARD_AUX,		250,	SENSOR_GROVE_OLED,			"GR_OLED",		"Groove OLED",					0,		true,		false,		1,			},
			// Add New Sensor Here!!!
			OneSensor { BOARD_AUX,		100,	SENSOR_SCD4x_CO2,			"SCD4x_CO2",		"SCD4x CO2", 					162,		true,		true,		1,			"ppm"},
			OneSensor { BOARD_AUX,		100,	SENSOR_SCD4x_TEMP,			"SCD4x_TEMP",		"SCD4x Temperature", 				163,		true,		true,		1,			"C"},
			OneSensor { BOARD_AUX,		100,	SENSOR_SCD4x_HUM,			"SCD4x_HUM",		"SCD4x Humidity", 				164,		true,		true,		1,			"%"},

			// Calypso ULP Anemometer
			OneSensor { BOARD_AUX,		100,	SENSOR_WIND_DIR,			"WIND_DIR",		"Wind Direction", 				165,		true,		false,		1,			"R"},
			OneSensor { BOARD_AUX,		100,	SENSOR_WIND_SPEED,			"WIND_SPEED",		"Wind Speed", 				166,		true,		false,		1,			"m/s"},
			
			// Radeon ULP IR Rain Gauge
			OneSensor { BOARD_AUX,		100,	SENSOR_RAIN_ACC,			"RAIN_ACC",		"Rain Accumulation", 				167,		true,		false,		1,			"mm"},
			OneSensor { BOARD_AUX,		100,	SENSOR_RAIN_EVENTACC,			"RAIN_EVENTACC",		"Rain Event Acc", 				168,		true,		false,		1,			"mm"},
			OneSensor { BOARD_AUX,		100,	SENSOR_RAIN_TOTALACC,			"RAIN_TOTALACC",		"Rain Total Acc", 				169,		true,		false,		1,			"mm"},
			OneSensor { BOARD_AUX,		100,	SENSOR_RAIN_INTERVAL,			"RAIN_INTERVAL",		"Rain Per Interval", 				170,		true,		false,		1,			"mm/h"},
